...
| :-------------  |:------------- |
API&nbsp;Signing&nbsp;Key&nbsp;Pair | An API signing key pair that is used to mint access tokens - create one in the console [here](https://www.twilio.com/console/project/api-keys)
TwiML&nbsp;App&nbsp;SID | The TwiML application with a voice URL configured to access your server running this app - create one [in the console here](https://www.twilio.com/console/voice/twiml/apps). Also, you will need to configure the Voice "REQUEST URL" on the TwiML app to point to a Twilio Function you will create.
...
You can add new functions with the plus button on that screen. Look for the *Twilio Client Quickstart* template, and create it. You'll need the four values you gathered above as the configuration settings for this template.

After you successfully put in the configuration, you'll have two Twilio Functions - one for the access token, and one for the TwiML web application's voice response.
...
Also, you'll need to modify the function URL in quickstart.js to point to your access token Function - go ahead and copy the access token URL like you did for the voice calls URL, and put that into quickstart.js, replacing this line:

```
https://YOUR_FUNCTION_SUBDOMAIN_HERE.twilio.io/access-token
```

The subdomain you use for Twilio Functions will be the same for any functions you create - including the access token and voice call functions.
...
[Visit our guide](https://www.twilio.com/docs/iam/access-tokens) to learn how to generate access tokens in your own C#, Java, Node.js, PHP, Python, or Ruby application.
